...
class ConsultantController extends AdminBaseController {
    protected $pro_data;
    protected $contractor;
    public function __construct(Contractor $contractor){

        parent::__construct();
        $this->contractor = $contractor->consultants();
        $this->middleware(function ($request, $next) {
            restrictEngineers($this->user_info->type_flag);
            $this->pro_data['title'] = 'Consultant';
            return $next($request);
        });
    }
...
//        abort_unless(Auth::User()->implementingOffice->isMonitoring==1, 403);
        ...
        return redirect()->route('consultant.create');
    }


    public function show(Contractor $consultant){
        $this->pro_data['authorizedPersons']=AuthorizedPerson::select(DB::raw('CONCAT(name, " - ",email," - ",mobile) AS full_name'), 'id')
            ->orderBy('name','asc')
            ->pluck('full_name', 'id');

        $this->pro_data['contractor'] = $consultant;
        return view('admin.contractor.show', $this->pro_data);
    }

    public function edit(Contractor $consultant){
//        abort_unless(Auth::User()->implementingOffice->isMonitoring==1, 403);
        $this->pro_data['contractor'] = $consultant;
        return view('admin.contractor.edit', $this->pro_data);
    }

    public function update(CreateContractorRequest $request, Contractor $consultant){
//        abort_unless(Auth::User()->implementingOffice->isMonitoring==1, 403);
        $status = $request->get('status') == 'on'?1:0;
        $image = $request->file('image');
        $imageName = $consultant->image;
        if($image != null && $image != '' && !empty($image) && isset($image)){
            $imageName = makeImageName($image->getClientOriginalName());
            $this->uploadImage('contractor', $imageName, $image);
        }
        ...
        $oldContractor=$consultant->toArray();
        $consultant->fill([
            'name'=>$request->get('name'),
            'nep_name'=>$request->get('nep_name'),
            'address'=>$request->get('address'),
        ...
        $email=$request->get('email')==null?$consultant->slug.'@contractordudbc.com':$request->get('email');

        $status=$request->get('email')==null?0:1;

        try{
            $consultant->myUser->fill([
                'name'=>$request->get('name'),
                'email'=>$email,
                'updated_by'=> Auth::user()->id,
                'status'=>$status,
        ...
        $change=logDescriptionUpdate($consultant, $oldContractor);
        if($change!=false){
            storeLog(null,$change,1 ,'Contractor');
        }
        session()->flash('update_success_info','" consultant named '.$request->get('name').'"');
        return redirect()->route('consultant.index');
    }

    public function destroy(Contractor $consultant){
        $name = $consultant->name;
        if(Input::get('hardDelete')){
            $consultant->forceDelete();
            $consultant->myUser->forceDelete();
        }else{
            $consultant->delete();
        }
        storeLog(null,$name,2 ,'Contractor');
        session()->flash('delete_success_info','" consultant named '.$name.'"');
        return redirect()->route('consultant.index');
    }
}
