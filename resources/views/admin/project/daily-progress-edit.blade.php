...
@section('content')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="{{route('project.show',$project->id)}}"><i
                        
                        class="fa fa-edit"></i> {{$project->name}}</a></li>
    
        <li class="active">
            <a href="{{route('daily.progress',$project->id)}}"><i class="fa fa-plus"></i> Add Daily Progress</a>
        </li>
        <li class="active">
            <a href="{{route('report.daily.progress',$project->id)."?from={$daily_progress->date}&to={$daily_progress->date}"}}"><i class="fa fa-eye"></i> View Daily Progress</a>
        </li>
    </ol>
    ...
    <div class="container-fluid">
        <h1 style="margin-top:5px">{{$project->name}} - {{ $setting->project_code}}
            ...
            <i class="h4 text-danger">[Last Updated At: {{isset($daily_progress) ? dateBS(date('Y-m-d', strtotime($daily_progress->updated_at))) : ""}}]</i>
            ...
            <span  style="float: right; margin-right: 200px;">
                @if(!$editable_daily_progress->isEmpty())
                    ...
                    @foreach($editable_daily_progress as $progress)
                        <a href="{{ route('daily.progress.edit', $progress->id) }}" title="Edit DailyProgress of {{ \Carbon\Carbon::now()->format('Y-m-d') == $project->date ? 'Today' : $progress->date }}">
                           
                                @if(isset($daily_progress))
                                @if($daily_progress->date == $progress->date)
                                    <i class="h4 text-danger" style="text-decoration: underline;" > <strong> {!! $progress->date !!} </strong></i>
                                @else
        ...
        {!!Form::open(['route'=>['daily.progress.update', $daily_progress->id],'method'=>'post','class'=>'showSavingOnSubmit form-horizontal','files'=>true])!!}
        
        {{--Date--}}
        <div class="row" style="margin-bottom: 20px;">
            <div class="col-lg-2 col-md-2 pull-right @if($errors->has('file')) has-error @elseif(count($errors->all())>0) has-success @endif has-feedback">
                <label for="date"> Date of this Daily Progress </label>
                <input  name="date" type="text" class="form-control nepaliDate" value="{{ $daily_progress->date }}" id="date" />
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body collapse {{ app()->environment() =='local' ? 'in' : ''}}" id="pro_collapse_manpower">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body collapse {{ app()->environment() =='local' ? 'in' : ''}}" id="pro_collapse_material">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body collapse {{ app()->environment() =='local' ? 'in' : ''}}" id="pro_collapse_weather">
                <div class="form-group">
                ...
                <div class="col-sm-4">
                    <select name="" id="work_activity_code">
                        ...
                        @foreach($activities as $activity)
                            <option value="{{ $activity->id }}" data-activity-title="{{ $activity->title}}" code="{{ $activity->code }}"
                                    data-activity-rate="{{ $activity->unit }}">{{ $activity->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-sm-4">
                    ...
                    @if(isset($daily_progress->activities) && $daily_progress->activities && count($daily_progress->activities))
                        @foreach($daily_progress->activities as $activity_index => $activity)
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-1" style="width:3%"><strong>{{ $activity_index+1 }}</strong></div>
                                ...
                                <div class="col-sm-2">
                                    <select class="form-control" name="activities[block][]">
                                        @foreach($blocks as $id=>$block_name)
                                            <option value="{{$id}}" {{ $activity_block ? ($activity_block->id == $id ? 'selected' : '') : ''}}>{{$block_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                
                                <div class="col-sm-1"><input type="number" name="activities[q][]" class="form-control"
                                                             min="1" value="{{ $activity['q']??null }}"></div>
                                <div class="col-sm-1"><input type="number" name="activities[progress][]"
                                                             class="form-control"
                                                             value="{{ $activity['progress']??null }}">
                                
                                </div><div class="col-sm-1"><input type="text" name="activities[remarks][]"
                                                                   class="form-control"
                                                                   value="{{ array_key_exists('remarks', $activity) ? $activity['remarks'] : null }}"></div>
                                <div class="col-sm-1">
        ...
        <div class="panel panel-default pro_news_detail">
            <div class="panel-heading">
                ...
                <button type="button" class="btn btn-xs btn-danger pull-right showToolTip" onclick="removeImages()" title="Remove All Images/Videos"><span class="glyphicon glyphicon-minus"></span></button>
                <button type="button" class="btn btn-xs btn-warning pull-right showToolTip" onclick="addImages()" title="Add New Image/Video"><span class="glyphicon glyphicon-plus"></span></button>
                {!! minimizeButton('pro_collapse_a_log') !!}
            </div>
            
            <br>
            
            <div class="panel-body pro_images_browser collapse {{ app()->environment() =='local' ? 'in' : ''}}" id="pro_collapse_a_log">
                <?php $i = 1;  ?>
                    ...
                    @foreach($activityLog->ActivityLogFiles as $media)
                        <div class="col-md-2 col-lg-2">
                            ...
                            @if(in_array($extension,$imageExtensions))
                                <img src="{{asset('public/activityFiles/'.$media->file_path)}}" width="300" alt="{{$media->name}}" title="{{$media->title}}" class="img img-responsive"/><br><br>
                            @else(in_array($extension,$videoExtensions))
                                <video width="280" height="200" controls>
                                    <source src="{{asset('public/activityFiles/'.$media->file_path)}}" type="video/mp4">
                                    <source src="{{asset('public/activityFiles/'.$media->file_path)}}" type="video/ogg">
                ...
                <div class="col-md-2 col-lg-2 pro_images_section">
                    ...
                    <label for="file{{$i}}"><span class="glyphicon glyphicon-plus"></span> Add a Image/Video</label>
                    <input type="file" name="file[]" class="form-control col-md-3 col-lg-3" id="file{{$i}}" onchange="readURL(this)" multiple />
                    ...
                    <input type="text" name="description[]" class="form-control col-md-3 col-lg-3" placeholder="Add Description" />
                    <div class="clearfix"></div>
                </div>
            </div>
            <div class="panel-footer">
                <button class="btn btn-sm btn-success" type="submit">Update</button>
                <!--pro_batch_lands payment_status 1->0-->
            </div>
        </div>
        ...
        <div class="div-stock-existing-files">
            <select name="type[]" id="" data-placeholder="Add Type..." data-tags="true" class="selectize select-stock-existing-files">
                <option value="">&nbsp;</option>
                @foreach($existing_files as $file)
                    @if($file->type)
                        <option value="{{$file->type}}">{{$file->type}}</option>
...
@section('footerContent')
    ...
    <script type="text/javascript">
        ...
        function addImages(){
            ...
            $('.pro_images_section:last-child').after('<div class="col-md-2 col-lg-2 pro_images_section"> <span class="glyphicon glyphicon-remove-circle closeButton" onclick="hideDiv($(this))"></span> <label for="file'+$i+'"><span class="glyphicon glyphicon-plus"></span> Add a Image</label> <input type="file" name="file[]" class="form-control col-md-3 col-lg-3" id="file'+$i+'" onchange="readURL(this)" /> ' + div_stock_existing_files.html() + '<br><br><input type="text" name="description[]" class="form-control col-md-3 col-lg-3 file-desc" placeholder="Add Description" /><input type="hidden" name="oldImages[]" value="0" /> <div class="clearfix"></div></div>');
    ...
    <script>
            ...
            if ($($this).find('option:selected').length > 0) {
                ...
                selected.map(function () {
                    ...
                    if (!$('#' + id).length) {
                        ...
                        var checkbox = '<div class="col-sm-1 btn-group btn-group-toggle" data-toggle="buttons" id="' + id + '" >' +
                            person.substring(0, 15) + '...<br>' +
                            '<label class="btn btn-default active present">' +
                            '<input type="radio" name="manpower[' + associated_with + '][' + title + '][attendence][' + this.value + ']" autocomplete="off" checked value="1" multiple> P' +
                            '</label>' +
                            '<label class="btn btn-default absent">' +
                            '<input type="radio" name="manpower[' + associated_with + '][' + title + '][attendence][' + this.value + ']" autocomplete="off" value="0" multiple> A' +
                            '</label>' +
                            '</div>'
        ...
        function add_work_activity($this, description, rate) {
            ...
            var html = '<div class="row">' +
                ...
                '                                <div class="col-sm-1" style="width:3%"><strong>'+ serial_no +'</strong></div>' +
